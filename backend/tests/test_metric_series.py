...
async def _battery_history(db, owner, values):
    """werte = [(days back, value)], the youngest one last."""
    for back, value in values:
        await metrics.record(db, owner.id, "akku.shelter", value, unit="%", ts=_days(back))
    await db.commit()
    return await metrics.series(db, owner.id, "akku.shelter")
...
async def test_the_line_fit_matches_the_real_history(db):
    ...
    r = await _battery_history(db, anna, [(22, 65), (18, 57), (14, 50), (10, 46), (6, 37),
                                       (2, 27), (0, 25)])
...
async def test_too_few_points_yield_no_forecast(db):
    ...
    anna = await make_user(db, "anna")
    r = await _battery_history(db, anna, [(2, 50), (0, 40)])
...
async def test_a_rising_series_has_no_end(db):
    anna = await make_user(db, "anna")
    r = await _battery_history(db, anna, [(4, 20), (3, 30), (2, 40), (0, 60)])
    state = await metrics.trend(db, r, target=0.0)
...
async def test_the_early_warning_comes_exactly_once(db):
    anna = await make_user(db, "anna")
    r = await _battery_history(db, anna, [(22, 65), (14, 50), (6, 37), (0, 25)])
    state = await metrics.trend(db, r, target=0.0)
    assert metrics.forewarn(r, state["days_left"], 20) is True
    assert metrics.forewarn(r, state["days_left"], 20) is False, "not the same thing twice"


async def test_a_fresh_battery_may_warn_again(db):
    anna = await make_user(db, "anna")
    r = await _battery_history(db, anna, [(22, 65), (14, 50), (6, 37), (0, 25)])
    metrics.forewarn(r, 5.0, 7)
    ...
    assert metrics.forewarn(r, 5.0, 7) is True
...
async def test_the_action_writes_and_reads_back(db):
    anna = await make_user(db, "anna")
    await _battery_history(db, anna, [(22, 65), (14, 50), (6, 37)])
...
async def test_nonsensical_values_do_not_enter_the_series(db):
    ...
    """
    anna = await make_user(db, "anna")
    await _battery_history(db, anna, [(22, 65), (14, 50), (6, 37), (0, 25)])
    inst = await _instance(db, anna)
    node = {"id": "m", "type": "auto_action", "data": {"config": {"action": {
    ...
    r2 = await metrics.series(db, anna.id, "akku.shelter")
...
async def test_values_from_a_few_minutes_yield_no_daily_trend(db):
    ...
    await db.commit()
    r = await metrics.series(db, anna.id, "spannung")
...
async def test_a_missing_value_is_skipped(db):
    ...
    """
    anna = await make_user(db, "anna")
    await _battery_history(db, anna, [(6, 40), (3, 34), (0, 30)])
    inst = await _instance(db, anna)
    ...
    series = await metrics.series(db, anna.id, "akku.shelter")
    assert series.last_value == 30.0
    assert (await metrics.trend(db, series))["points"] == 3, "no point was added"
...
async def test_the_trend_names_the_age_of_the_last_value(db):
    anna = await make_user(db, "anna")
    r = await _battery_history(db, anna, [(9, 60), (6, 50), (3, 40)])
    state = await metrics.trend(db, r)
...
async def test_the_age_even_without_a_line_fit(db):
    ...
    anna = await make_user(db, "anna")
    r = await _battery_history(db, anna, [(5, 50), (4, 45)])
    state = await metrics.trend(db, r)
...
async def test_silence_and_remaining_runtime_do_not_swallow_each_other(db):
    ...
    anna = await make_user(db, "anna")
    r = await _battery_history(db, anna, [(22, 65), (14, 50), (6, 37), (3, 25)])
    state = await metrics.trend(db, r, target=0.0)
    assert metrics.forewarn(r, state["days_left"], 30) is True
...
async def test_a_fresh_series_does_not_stay_quiet(db):
    anna = await make_user(db, "anna")
    await _battery_history(db, anna, [(0.2, 80), (0.1, 79), (0, 78)])
    inst = await _instance(db, anna)
    node = {"id": "lesen", "type": "auto_action", "data": {"config": {"action": {
...
async def test_numbers_may_come_from_the_context(db):
    ...
    """
    anna = await make_user(db, "anna")
    await _battery_history(db, anna, [(9, 60), (6, 50), (3, 40)])
    inst = await _instance(db, anna)
...
async def test_points_come_with_an_id_and_a_trend(client, db):
    ...
    data = r.json()
    assert [p["value"] for p in data["points"]] == [60.0, 50.0, 40.0]
    assert all(p["id"] for p in data["points"]), "deleting needs the id"
    assert data["trend"]["per_day"] < 0
...
async def test_the_span_also_applies_to_the_line_fit(client, db):
    ...
    anna = await make_user(db, "anna")
    await _battery_history(db, anna, [(40, 100), (39, 99), (38, 98), (3, 40), (2, 38), (1, 36)])
    eng = (await client.get("/metrics/akku.shelter/points?days=7", headers=auth(anna))).json()
    wide = (await client.get("/metrics/akku.shelter/points?days=90", headers=auth(anna))).json()
    assert len(eng["points"]) == 3 and len(wide["points"]) == 6
    assert eng["trend"]["per_day"] != wide["trend"]["per_day"]
...
async def test_removing_a_single_outlier(client, db):
    ...
    anna = await make_user(db, "anna")
    await _battery_history(db, anna, [(6, 60), (4, 50), (2, 40)])
    await metrics.record(db, anna.id, "akku.shelter", 999.0)     # the outlier, last
    await db.commit()
    data = (await client.get("/metrics/akku.shelter/points", headers=auth(anna))).json()
    bad = [p for p in data["points"] if p["value"] == 999.0][0]

    path = await client.delete(f"/metrics/akku.shelter/points/{bad['id']}",
                              headers=auth(anna))
    assert path.status_code == 204
    r = await metrics.series(db, anna.id, "akku.shelter")
    await db.refresh(r)
    assert r.last_value == 40.0, "the head of the series moves up"
    after = (await client.get("/metrics/akku.shelter/points", headers=auth(anna))).json()
...
async def test_a_foreign_value_is_not_deleted(client, db):
    ...
    await db.commit()
    mine = (await client.get("/metrics/meins/points", headers=auth(anna))).json()
    pid = mine["points"][0]["id"]
