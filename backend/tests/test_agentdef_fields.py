...
def test_agent_definition_carries_context_limit():
...
def test_run_reads_only_existing_agent_fields():
...
async def test_watchdog_reports_standstill_exactly_once(monkeypatch, caplog):
...
async def test_model_catalog_carries_context_and_speed(db):
...
async def test_model_fetch_does_not_overwrite_curated_names(db, monkeypatch):
