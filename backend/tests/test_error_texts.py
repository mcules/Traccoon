...
def test_placeholders_and_values_match():
    ...
    error = []
    for file, line, args, names in _calls():
        ...
        placeholder = set(re.findall(r"\{(\w+)\}", text.value))
        if placeholder != set(names):
            error.append(f"{file}:{line}: {sorted(placeholder)} != {sorted(names)}")
    assert not error, "\n".join(error)
...
def test_same_key_same_text():
    ...
    seen: dict[str, str] = {}
    error = []
    for file, line, args, _names in _calls():
...
def test_values_land_in_the_text():
    f = Error(status.HTTP_404_NOT_FOUND, "err.test", "No {what} for {who}", what="board", who="anna")
    assert f.detail == "No board for anna"
    assert f.values == {"what": "board", "who": "anna"}
