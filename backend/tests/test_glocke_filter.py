...
async def test_ohne_versand_bleibt_visible(db, client):
...
async def test_assistent_und_spam_haengen_am_eigenen_state(db, client):
    anna = await make_user(db, "anna")
...
async def test_all_gelesen_raeumt_auch_das_verborgene(db, client):
