...
handler found no `[TRA-1]` key in the quoted text and returned silently: no task, no
...
async def test_a_named_agent_lands_in_the_meta(db, anna, monkeypatch):
    """/uniwar goes the same way: the agent stands in the meta, otherwise the assistant runs."""
    import app.core.redis as redis_mod
