"""Lebenszyklus der Testumgebungen (TRA-18) — die Teile, die ohne Docker prüfbar sind:
...
async def test_complete_setzt_kein_fertig_bei_merge_konflikt(client, db, monkeypatch):
    owner = await make_user(db, "owner")
    proj = await make_project(db, "TST", "Test")
    ...
    # Kein Redis/Worker im Test: das Merge-Ergebnis vorgeben. `complete()` importiert
    # enqueue_task/wait_result erst im Funktionsrumpf — Patchen am Modul greift daher.
    import app.core.redis as redismod

    async def fake_enqueue(payload):
        return None

    async def fake_wait(task_id, timeout=1800.0, poll=0.4):
        return {"status": "conflict", "error": "Merge-Konflikt: app.py"}

    monkeypatch.setattr(redismod, "enqueue_task", fake_enqueue)
    monkeypatch.setattr(redismod, "wait_result", fake_wait)
...
async def test_complete_setzt_fertig_bei_sauberem_merge(client, db, monkeypatch):
    owner = await make_user(db, "owner")
    proj = await make_project(db, "TST", "Test")
    m = await add_member(db, proj, owner, ProjectRole.owner)
    m.ai_assign = True
