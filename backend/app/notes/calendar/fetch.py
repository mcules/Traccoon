...
def _local(value: dt.datetime | dt.date) -> dt.datetime:
    """An occurrence as a wall clock here.

    A time with a zone is converted; one without is already local. A date is a
    day, and a day starts at midnight wherever it is read.
    """
...
def expand(ics_text: str, calendar: str, first: dt.date, last: dt.date) -> list[Event]:
...
async def refresh(sources: list[Source], *, days_back: int = 120,
    ...
    for source in sources:
        try:
            ...
            events.extend(expand(text, source.name, first, last))
