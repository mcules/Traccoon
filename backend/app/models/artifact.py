...
Below it hangs the field model following the ALMEX example (`Artifacts → Fields → Values`):
...
drag the foreign keys from `issues`, `hardware_assets` and `workflow_instances` along:
    ...
    assigned value      ArtifactValue / artifact_values         TRA-29 → high
    the thing itself    Artifact / artifacts                    TRA-29, HW-4
"""
