...
async def _waiting_approval(db: AsyncSession, issue: Issue) -> tuple[int, WorkflowStepRun]:
    ...
    if inst is None:
        raise Error(status.HTTP_409_CONFLICT, "err.no_process_for_ticket",
    ...
    if step is None:
        raise Error(status.HTTP_409_CONFLICT, "err.no_approval_waiting_right_now",
...
async def start_planning(
    ...
    if issue.assigned_agent is None:
        raise Error(status.HTTP_409_CONFLICT, "err.no_agent_assigned", "No agent assigned")
    ...
    if inst is None:
        raise Error(status.HTTP_409_CONFLICT, "err.no_published_lifecycle_process_project",
...
async def approve_plan(
    ...
    if issue.agent_status != TicketAgentStatus.plan_review:
        raise Error(status.HTTP_409_CONFLICT, "err.ticket_not_plan_approval",
                     "The ticket is not in plan approval")
    if not issue.plan:
        raise Error(status.HTTP_409_CONFLICT, "err.no_plan_present", "No plan present")
...
async def approve_split(pair: tuple[Issue, Access] = Depends(get_issue_access),
    ...
    if issue.hold_reason != HoldReason.plan_split:
        raise Error(status.HTTP_409_CONFLICT, "err.no_splitting_proposal_approve",
    ...
    if not children:
        raise Error(status.HTTP_409_CONFLICT, "err.process_created_no_sub_tasks_check_plan",
...
async def complete(
    ...
    if issue.agent_status not in (TicketAgentStatus.to_test, TicketAgentStatus.testing,
        ...
        raise Error(status.HTTP_409_CONFLICT, "err.ticket_not_ready_acceptance",
