...
async def list_locations(_: User = Depends(get_current_user), db: AsyncSession = Depends(get_session)):
...
async def create_location(
    data: LocationIn, _: User = Depends(get_current_user), db: AsyncSession = Depends(get_session)
):
    loc = Location(name=data.name, type=data.type, parent_id=data.parent_id, notes=data.notes)
    db.add(loc)
    await db.flush()
...
async def update_location(
    ...
    loc.name, loc.type, loc.parent_id, loc.notes = data.name, data.type, data.parent_id, data.notes
    await db.flush()
    loc.full_path = await _compute_full_path(loc, db)
...
async def delete_location(
    loc_id: int, _: User = Depends(get_current_user), db: AsyncSession = Depends(get_session)
):
    loc = await db.get(Location, loc_id)
    if loc is None:
        raise HTTPException(404, "Ort nicht gefunden")
...
async def asset_steps(
    asset_id: int, _: User = Depends(get_current_user), db: AsyncSession = Depends(get_session)
):
    a = await db.get(HardwareAsset, asset_id)
    if a is None:
        raise HTTPException(404, "Exemplar nicht gefunden")
