...
# Voice messages are transcribed in `services/transcribe.py`, the same code the API route for
# the phone app uses. The names stay here so that the handlers and the tests read as before.
from ..services import transcribe as speech  # noqa: E402

WHISPER_URL = speech.WHISPER_URL
ASR_URL = speech.ASR_URL
VOICE_MAX_SECONDS = speech.VOICE_MAX_SECONDS
VOICE_MAX_BYTES = speech.VOICE_MAX_BYTES
_transcribe = speech.transcribe
