...
def _cuttable(m: dict) -> bool:
    ...
    without memory, starts from the beginning). On exactly that, UNI-4 hung for two full
...
async def handover(db, *, messages: list[dict], reason: str, last_text: str,
    ...
    was not even enough to know which files had already been read: UNI-12 started three runs
