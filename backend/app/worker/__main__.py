...
async def handle(job: dict, redis: Redis) -> None:
    ...
    if kind == "accept":
        ...
        # nur bei sauberem Merge auf „Fertig" setzen (ABC-18).
        try:
...
# Gesprächsverlauf im Chat (ABC-30): Ein Chat war bisher eine Folge voneinander unabhängiger
...
# (`chat_summaries`) — Vorbild ist die Kontext-Kompaktierung von Predecessor.
CHAT_HISTORY_MAX = 8
...
async def _chat_history(db, t) -> list[dict]:
    ...
    # Fach-Agenten führen eigene Gespräche — der GameProj-Operator hat mit dem Assistenten nichts zu tun.
...
async def _handle_assistant_task(job: dict, redis: Redis) -> None:
    ...
    async with SessionLocal() as db:
        ...
        if melden:
            ...
            # Assistenten und die eines Fach-Agenten (z. B. gameproj-operator) nicht zu unterscheiden.
...
async def _handle_curator(job: dict) -> None:
    ...
    Predecessor stößt seinen Curator bei Untätigkeit an. Hier ist der Auslöser das Ende eines
...
# 2026-08-07 zweimal ABC-31, jeweils nach knapp 40 Zügen Arbeit.
...
def _signale_annehmen() -> None:
    ...
    rettet den Auftrag, nicht das Gespräch: am 2026-08-07 kostete das ABC-31 zweimal knapp
    40 Züge Arbeit.
    """
