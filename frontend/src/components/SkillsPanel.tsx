    ...
    <div>
      ...
      <div className="mb-4 space-y-2">
        {skills?.map((s) => (
          <div key={s.id} className="flex items-center gap-3 rounded border border-line bg-card p-2 text-sm">
            ...
            <button onClick={() => del.mutate(s.id)} className="text-muted hover:text-red-400">{tr("common.loeschen_klein")}</button>
          </div>
        ))}
      ...
      <div className="space-y-2 rounded-lg border border-line bg-card p-3">
        <div className="flex items-center gap-2">
          ...
          <label className="flex items-center gap-1 text-sm"><input type="checkbox" checked={autostart} onChange={(e) => setAuto(e.target.checked)} />{tr("skills_panel.auto")}</label>
        </div>
