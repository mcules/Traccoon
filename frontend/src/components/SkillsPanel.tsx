...
export default function SkillsPanel() {
  ...
  return (
    <div>
      <p className="mb-3 text-sm text-muted">Wiederverwendbare Prompt-Bausteine (versioniert). Agenten laden sie
        über „allowed_skills". autostart = dauerhaft aktiver Modus.</p>
      <div className="mb-4 space-y-2">
        ...
        {skills?.length === 0 && <div className="text-xs text-muted">Keine Skills.</div>}
      </div>
      <div className="space-y-2 rounded-lg border border-line bg-card p-3">
        <div className="flex items-center gap-2">
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="Name (z. B. Test-Driven Development)" className="flex-1 rounded border border-line bg-surface px-2 py-1.5 text-sm" />
        ...
        <textarea value={body} onChange={(e) => setBody(e.target.value)} rows={3} placeholder="Skill-Text (Markdown)" className="w-full rounded border border-line bg-surface px-2 py-1.5 text-sm" />
