  ...
  return (
    <Area>
      ...
      {(myMcp?.available?.length ?? 0) > 0 && (
        <div className="mb-4 flex flex-wrap items-center gap-2 rounded-lg border border-line bg-surface p-3">
          <div className="text-sm">
            <span className="font-medium text-ink">🔌 {tr("mcp_panel.count_mcp_servers_available", { count: myMcp.available.length })}</span>
            {myMcp?.provisioned && <span className="ml-2 rounded bg-yellow-500/15 px-1.5 text-xs text-yellow-400">{tr("mcp_panel.gateway_group_active")}</span>}
            <p className="text-xs text-muted">{tr("mcp_panel.take_them_over_as_real_editable_server_entrie")}</p>
          </div>
          <div className="flex-1" />
          <button onClick={() => importMcp.mutate()} disabled={importMcp.isPending}
            className={BUTTON.primary}>
            {tr(importMcp.isPending ? "mcp_panel.importing" : "mcp_panel.import_servers")}</button>
        </div>
      )}

      <p className="mb-3 text-sm text-muted">{tr("mcp_panel.own_tool_servers_agents")}</p>
      <p className="mb-3 text-xs text-yellow-400">{tr("mcp_panel.only_http_sse_served")}</p>
      <Errorrow text={err} />
          ...
          <ListenLine key={m.id}>
            ...
            <div className="flex min-w-0 flex-1 flex-wrap items-center gap-x-2">
              ...
              {(m.variables?.length ?? 0) > 0 && (
                <span className="rounded bg-surface px-1 text-xs">{tr("mcp_panel.count_variable_s", { count: m.variables.length })}</span>
              )}
              {m.enabled && <span className="text-xs text-green-400">{tr("mcp_panel.active")}</span>}
            </div>
            <Actions>
      ...
      <button onClick={() => { setErr(""); setDialog({ ...EMPTY, variables: [] }); }}
        className={BUTTON.primary}>
        {ICON.fresh} {tr("mcp_panel.new_server")}
      </button>
...
function ServerDialog({ start, error: error, runs: running, onClose, onSave }: {
  ...
  return (
    <Dialog wide title={form.id ? tr("mcp_panel.edit_server") : tr("mcp_panel.new_server")} onClose={onClose}
      foot={<DialogFoot onCancel={onClose} disabled={!form.name.trim()} runs={running}
        onSave={() => onSave(form)}
        saveText={form.id ? undefined : tr("common.create")} />}>
      <Errorrow text={error} />
      <div className="space-y-3">
        ...
        <div>
          <div className="mb-1 text-xs font-medium text-muted">{tr("mcp_panel.variables_filled_per_instance_applied_as_head")}</div>
