...
import { tr } from "../../i18n";
import { Bereich, Etikett, Liste, ListeLeer, ListenZeile } from "../ui";
...
export default function AblagenPanel() {
  ...
  return (
    <Bereich hinweis={tr("ablagen.einleitung")}>
      <Liste>
        {ablagen.length === 0 && <ListeLeer>{tr("ablagen.keine")}</ListeLeer>}
        {ablagen.map((a) => (
          <ListenZeile key={a.id} spalten="sm:grid-cols-[minmax(0,1fr)_10rem_auto]"
