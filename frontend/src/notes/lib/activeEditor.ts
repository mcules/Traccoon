 ...
 * Returns false when there is no editor to insert into, so a caller can fall
 * back instead of silently dropping what it meant to write.
 */
export function fmtInsert(text: string, caretOffset = text.length): boolean {
  const v = active;
  if (!v) return false;
  const { from, to } = v.state.selection.main;
