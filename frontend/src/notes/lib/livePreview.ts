...
// Per-vault property type registry, the same one the predecessor kept.
...
/**
 * Start the predecessor's "Add file property" flow: focus a new property-key field in
...
// Small "external link" glyph appended after outgoing http(s) links, like the predecessor.
...
class WikilinkWidget extends WidgetType {
  /** `aria` = default display text, set when an alias overrides it (like the predecessor). */
...
class CheckboxWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    box.onmousedown = (e) => {
      ...
      // Same semantics as ticking the box in a query result: the done date and
      // the next instance of a recurring task come from the plugin settings, so
      // the rule lives on the server and is asked for here (nothing is written
      // there — the edit goes through this document).
      void api
        .dvTaskLines({ line: line.text, checked, mode: 'tasks' })
        .then(({ lines }) => {
          const cur = view.state.doc.lineAt(this.pos);
          if (cur.text !== line.text) return; // line moved on: leave it alone
          view.dispatch({ changes: { from: cur.from, to: cur.to, insert: lines.join('\n') }, userEvent: 'input' });
        })
        .catch(() => {
          // Offline or an unexpected line: at least flip the box.
 ...
 * The predecessor size param: `![[img|W]]` for wikilink embeds, `![alt|W](url)` for
...
class ImageWidget extends WidgetType {
  /** `width`/`height` from the predecessor's size param `![[img|300]]` / `![[img|300x200]]`. */
  ...
  toDOM(view: EditorView) {
    ...
    // Missing attachment → like the predecessor "could not be found" box.
    img.onerror = () => {
      img.remove();
    ...
    // keeping aspect ratio (height auto), like the predecessor's image resize.
...
/* ---------------- math (KaTeX, lazy-loaded like the predecessor's MathJax) ---------------- */
...
/* ---------------- note embeds (transclusion, like the predecessor ![[note]]) ---------------- */
...
let noteEmbedProvider: (target: string) => Promise<{ html: string; path?: string } | null> = async () => null;
export function setLivePreviewNoteEmbedProvider(
  fn: (target: string) => Promise<{ html: string; path?: string } | null>,
) {
...
/**
 * A drawing embedded in a note, in the editor.
 *
 * The same picture the reading view shows, drawn into a block the editor treats
 * as one object — clicking it opens the drawing, because that is the only thing
 * one can usefully do with a picture inside a text editor.
 */
class DrawingWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    box.className = 'excalidraw-embed cm-drawing-embed';
    box.dataset.drawing = this.target;
    measureOnResize(box, view);
    box.addEventListener('dblclick', () => openLink(this.target));
    renderDrawingEmbeds(box.parentElement ?? box);
    // The placeholder has to be in the document before it can be filled, and a
    // widget's DOM is not yet attached here; one frame later it is.
    requestAnimationFrame(() => renderDrawingEmbeds(box.parentElement ?? document.body));
    return box;
  }
}
...
class NoteEmbedWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    void noteEmbedProvider(this.target).then((res) => {
      ...
      // Queries inside an embed are queries too — and they run against the
      // embedded note, not the one showing it.
      renderDataviewBlocks(content, res.path ?? null);
...
/** A frontmatter value that is (or contains) a wikilink renders as a link in
 *  the predecessor's property list. Only links — a property value is not markdown. */
function propertyValueHtml(raw: string): string | null {
  if (!/\[\[[^\]]+\]\]/.test(raw)) return null;
  const escape = (t: string) => t.replace(/[&<>"]/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' })[c]!);
  return escape(raw).replace(/\[\[([^\]|#]+)(?:#[^\]|]+)?(?:\|([^\]]+))?\]\]/g, (_all, target: string, alias?: string) => {
    const label = alias || target.slice(target.lastIndexOf('/') + 1);
    return `<a class="internal-link" href="#" data-wikilink="${target}">${label}</a>`;
  });
}

/** Show links while idle, the raw text while editing. */
function linkifyEditable(el: HTMLElement): void {
  const paint = () => {
    if (document.activeElement === el) return;
    const raw = el.dataset.raw ?? '';
    const html = propertyValueHtml(raw);
  ...
  el.addEventListener('blur', () => setTimeout(paint, 0));
  el.addEventListener('mousedown', (e) => {
    const a = (e.target as HTMLElement).closest('[data-wikilink]') as HTMLElement | null;
    if (!a) return;
    e.preventDefault(); // do not put the caret into the field
    e.stopPropagation();
...
/** Last line of a heading's section: everything up to the next heading of the
 *  same or a higher level. */
function headingLastLine(doc: EditorState['doc'], n: number): number | null {
  const m = doc.line(n).text.match(FOLD_HEADING_RE);
  if (!m) return null;
  const level = m[1].length;
  let last = n;
  for (let j = n + 1; j <= doc.lines; j++) {
    const h = doc.line(j).text.match(FOLD_HEADING_RE);
    if (h && h[1].length <= level) break;
    last = j;
  }
  // Trailing blank lines do not belong to the fold.
  while (last > n && !doc.line(last).text.trim()) last--;
  return last > n ? last : null;
}

/**
 * How many list levels deep a line sits.
 *
 * One tab is one level, four spaces are one level, and anything left over
 * counts as one more — which is how the same file gets drawn, and that is the
 * point: folding has to agree with what one sees. Counting characters instead
 * makes a child indented with two tabs shallower than its parent indented with
 * two spaces, so the fold stops early and leaves half the subtree standing.
 * Counting columns makes two tabs deeper than a tab and two spaces, so folding
 * one item swallows its siblings. Only this measure matches the picture.
 *
 * Exported for the fold, used by the renderer: one function, so the two cannot
 * drift apart again.
 */
export function listIndentUnits(ws: string): number {
  let n = 0;
  for (let i = 0; i < ws.length; i += ws[i] === '\t' ? 1 : 4) n++;
  return n;
}

/** The last line belonging to the item on line `n` — everything indented deeper,
 *  including blank lines that are followed by more of the item. */
function listItemLastLine(doc: EditorState['doc'], n: number): number | null {
  const m = doc.line(n).text.match(LIST_MARKER_RE);
  if (!m) return null;
  const indent = listIndentUnits(m[1]);
  const indentOf = (t: string) => listIndentUnits((t.match(/^\s*/) ?? [''])[0]);
  let last = n;
  for (let j = n + 1; j <= doc.lines; j++) {
    ...
    if (!text.trim()) {
      ...
      if (k > doc.lines || indentOf(doc.line(k).text) <= indent) break;
      j = k - 1;
      continue;
    }
...
class ListFoldArrow extends WidgetType {
  ...
  eq(o: ListFoldArrow) {
    return o.lineFrom === this.lineFrom && o.folded === this.folded && o.heading === this.heading;
  }
  ignoreEvent() {
    return true;
  }
  toDOM(view: EditorView) {
    const el = document.createElement('span');
    el.className = 'cm-list-fold' + (this.folded ? ' is-folded' : '') + (this.heading ? ' cm-heading-fold' : '');
    ...
    el.addEventListener('mousedown', (e) => {
      e.preventDefault();
      e.stopPropagation();
      view.dispatch({ effects: toggleListFold.of(this.lineFrom) });
    });
    return el;
  }
}

function buildListFolds(state: EditorState): DecorationSet {
  if (!state.field(livePreviewState, false)) return Decoration.none;
  const doc = state.doc;
  const folded = state.field(listFoldState, false) ?? [];
  const ranges: Range<Decoration>[] = [];
  for (let n = 1; n <= doc.lines; n++) {
    const line = doc.line(n);
    const isHeading = FOLD_HEADING_RE.test(line.text);
    const last = isHeading
      ? headingLastLine(doc, n)
      : LIST_MARKER_RE.test(line.text)
        ? listItemLastLine(doc, n)
        : null;
    if (last === null) continue; // nothing to fold
    const isFolded = folded.includes(line.from);
    ranges.push(
      Decoration.widget({ widget: new ListFoldArrow(line.from, isFolded, isHeading), side: -1 }).range(line.from),
    );
    if (isFolded) {
      ranges.push(Decoration.line({ class: 'cm-list-folded' }).range(line.from, line.from));
      ranges.push(Decoration.replace({ block: true }).range(line.to, doc.line(last).to));
      n = last;
    }
  }
...
export const listFoldDeco = StateField.define<DecorationSet>({
  create: (state) => buildListFolds(state),
  update(value, tr) {
    if (tr.docChanged || tr.effects.some((e) => e.is(toggleListFold) || e.is(setListFolds) || e.is(setLivePreviewEnabled))) {
      return buildListFolds(tr.state);
    }
...
/* ---------------- mermaid (lazy, like the predecessor) ---------------- */
...
/** Whether the editor has keyboard focus. A block only falls back to its source
 *  when the cursor is *deliberately* inside it; on load the cursor sits at
 *  offset 0 without focus, and a note starting with a query block should show
 *  the result, not its code. */
...
/** Flips once the user actually clicks or types in this note. Opening a note
 *  puts the cursor at offset 0 and focuses the editor, which must NOT count as
 *  "the cursor is inside the first block" — otherwise every daily note opens
 *  showing the source of its leading query block. */
export const userEngagedField = StateField.define<boolean>({
  create: () => false,
  update(value, tr) {
    for (const e of tr.effects) if (e.is(setNotePath)) return false;
    // Only real user input counts. Loading a note into the editor is a
    // programmatic transaction (docChanged, but no user event) — treating that
    // as engagement made every note open showing the source of its first block.
    if (tr.isUserEvent('select') || tr.isUserEvent('input') || tr.isUserEvent('delete')) return true;
    return value;
  },
});

export const editorFocusReporter = EditorView.domEventHandlers({
  focus: (_e, view) => {
    if (!view.state.field(editorFocusedField, false)) view.dispatch({ effects: setEditorFocused.of(true) });
    return false;
  },
  blur: (_e, view) => {
    if (view.state.field(editorFocusedField, false)) view.dispatch({ effects: setEditorFocused.of(false) });
    return false;
  },
});

/* ---------------- dataview / dataviewjs (rendered like the predecessor) ---------- */

/** The note the editor currently shows — dataview needs it for `this`. */
export const setNotePath = StateEffect.define<string>();

/** Last note path the editor was told about — table cells render inline
 *  dataview through `appendInline`, which has no access to editor state. */
let currentNotePath = '';

/** Editor tells us which note it is showing, for renderers without state. */
export function rememberNotePath(path: string): void {
  currentNotePath = path;
}

export const notePathField = StateField.define<string>({
  create: () => currentNotePath,
  update(value, tr) {
    for (const e of tr.effects)
      if (e.is(setNotePath)) {
        currentNotePath = e.value;
        return e.value;
      }
    return value;
  },
...
type QueryLang = 'dataview' | 'dataviewjs' | 'tasks';
...
class DataviewWidget extends WidgetType {
  ...
  eq(o: DataviewWidget) {
    return o.code === this.code && o.lang === this.lang && o.path === this.path && o.from === this.from;
  }
  // Clicks inside the result (links, task checkboxes, buttons) are the widget's
  // own business — CodeMirror must not turn them into editor input.
  ignoreEvent() {
    return true;
  }
  toDOM(view: EditorView) {
    const el =
      this.lang === 'dataviewjs'
        ? renderJsBlock(this.code, this.path || null)
        : this.lang === 'tasks'
          ? renderTasksBlock(this.code)
          : renderDqlBlock(this.code, this.path || null);
    el.classList.add('cm-dataview');
    measureOnResize(el, view);
    // Tell the document whether this block produced anything, so an empty one
    // can take the blank line after it with it.
    ...
    const report = () => {
      ...
      setTimeout(() => view.dispatch({ effects: markBlockEmpty.of({ from, empty }) }), 0);
    };
    if (typeof MutationObserver !== 'undefined') {
      const mo = new MutationObserver(() => {
        report();
        if (!el.isConnected) mo.disconnect();
      });
...
function buildDataview(state: EditorState): DecorationSet {
  if (!state.field(livePreviewState, false)) return Decoration.none;
  const doc = state.doc;
  const path = state.field(notePathField, false) ?? '';
  const ranges: Range<Decoration>[] = [];
  for (let n = 1; n <= doc.lines; n++) {
    const line = doc.line(n);
    const open = line.text.match(/^\s*(`{3,}|~{3,})\s*(dataviewjs|dataview|tasks)\s*$/i);
    if (!open) continue;
    let end = -1;
    for (let j = n + 1; j <= doc.lines; j++) {
      if (doc.line(j).text.trim().startsWith(open[1][0].repeat(3))) {
    ...
    // where live preview is off entirely and this function renders nothing.
    {
      ...
      let blockFrom = from;
      let blockTo = to;
      if ((state.field(emptyBlockState, false) ?? []).includes(from)) {
        // Nothing came out: swallow the blank lines around it as well, so the
        // note closes up instead of leaving a hole.
        let j = end;
        while (j < doc.lines && !doc.line(j + 1).text.trim()) j++;
        blockTo = doc.line(j).to;
        let i = n;
        while (i > 1 && !doc.line(i - 1).text.trim()) i--;
        blockFrom = doc.line(i).from;
      }
      ranges.push(
        Decoration.replace({ widget: new DataviewWidget(code, lang, path, from), block: true }).range(blockFrom, blockTo),
      );
    }
    n = end;
  }
...
export const dataviewField = StateField.define<DecorationSet>({
  create: (state) => buildDataview(state),
  update(value, tr) {
    if (
      tr.docChanged ||
      tr.selection ||
      tr.effects.some(
        (e) =>
          ...
          e.is(setNotePath) ||
          e.is(setEditorFocused) ||
          e.is(markBlockEmpty),
      )
    ) {
      return buildDataview(tr.state);
    }
    return value.map(tr.changes);
  },
  provide: (f) => EditorView.decorations.from(f),
 ...
 * into the document and fill in a moment later. CodeMirror measured them while
 * they were still flat, so the document's height map stayed wrong and the
...
/** A block that rendered nothing reports itself, so the blank line after it can
 *  be swallowed as well — otherwise an empty block still leaves a gap. */
export const markBlockEmpty = StateEffect.define<{ from: number; empty: boolean }>();

export const emptyBlockState = StateField.define<readonly number[]>({
  create: () => [],
  update(value, tr) {
    let v = tr.docChanged ? value.map((p) => tr.changes.mapPos(p, 1)) : [...value];
    for (const e of tr.effects) {
      if (e.is(markBlockEmpty)) {
        if (e.value.empty) {
          if (!v.includes(e.value.from)) v = [...v, e.value.from];
        } else v = v.filter((x) => x !== e.value.from);
      }
    }
    return v;
  },
...
/* ------------- inline dataview: `= query` spans and [key:: value] --------- */
...
class InlineQueryWidget extends WidgetType {
  ...
  eq(o: InlineQueryWidget) {
    return o.expr === this.expr && o.path === this.path;
  }
  ignoreEvent() {
    return true;
  }
  toDOM(view: EditorView) {
    const el = inlineQueryEl(this.expr, this.path || null);
    measureOnResize(el, view);
    return el;
  }
}
...
class InlineFieldWidget extends WidgetType {
  ...
  eq(o: InlineFieldWidget) {
    return o.key === this.key && o.value === this.value && o.wrapping === this.wrapping;
  }
  ignoreEvent() {
    return true;
  }
  toDOM() {
    return inlineFieldEl(this.key, this.value, this.wrapping);
  }
}

const INLINE_CODE_RE = /`([^`\n]+)`/g;

function buildDataviewInline(state: EditorState): DecorationSet {
  if (!state.field(livePreviewState, false)) return Decoration.none;
  const settings = dataviewSettings();
  const path = state.field(notePathField, false) ?? '';
  const ro = state.field(livePreviewReadonly, false) ?? false;
  const engaged = (state.field(editorFocusedField, false) ?? false) && (state.field(userEngagedField, false) ?? false);
  const doc = state.doc;
  const ranges: Range<Decoration>[] = [];

  // The cursor sitting inside a rendered piece reveals its source, exactly like
  // the plugin does (`selectionAndRangeOverlap`).
  const touched = (from: number, to: number) => {
    if (ro || !engaged) return false;
  ...
  for (let n = 1; n <= doc.lines; n++) {
    ...
    if (fence) {
      if (fenceMatch && line.text.trim().startsWith(fence)) fence = null;
      continue;
    }
    if (fenceMatch) {
    ...
    if (settings.enableInlineDataview) {
      ...
      let m: RegExpExecArray | null;
      while ((m = INLINE_CODE_RE.exec(line.text))) {
        const expr = inlineQueryOf(m[1]);
        if (expr === null) continue;
        const from = line.from + m.index;
        const to = from + m[0].length;
        if (touched(from - 1, to + 1)) continue;
        ranges.push(Decoration.replace({ widget: new InlineQueryWidget(expr, path) }).range(from, to));
      }
    }

    if (settings.prettyRenderInlineFieldsInLivePreview) {
      const fieldRe = inlineFieldRegex();
      let f: RegExpExecArray | null;
      while ((f = fieldRe.exec(line.text))) {
        if ((f[1] === '[') !== (f[4] === ']')) continue;
        const from = line.from + f.index;
        const to = from + f[0].length;
        if (touched(from, to)) continue;
        ranges.push(
          Decoration.replace({ widget: new InlineFieldWidget(f[2], f[3], f[1] as '[' | '(') }).range(from, to),
        );
      }
    }
  }
...
export const dataviewInlineField = StateField.define<DecorationSet>({
  create: (state) => buildDataviewInline(state),
  update(value, tr) {
    if (
      tr.docChanged ||
      tr.selection ||
      tr.effects.some((e) => e.is(setLivePreviewEnabled) || e.is(setLivePreviewReadonly) || e.is(setNotePath) || e.is(setEditorFocused))
    ) {
      return buildDataviewInline(tr.state);
    }
    return value.map(tr.changes);
  },
  provide: (f) => EditorView.decorations.from(f),
...
/** Find GFM/the predecessor tables in the doc: a header row, a delimiter row, then body rows. */
...
// `<br>` is the only inline HTML the predecessor commonly renders inside table cells.
const CELL_INLINE_RE =
  /(<br\s*\/?>)|(`[^`]+`)|(\*\*[^*]+\*\*)|(\*[^*]+\*|_[^_]+_)|(!?\[\[[^\]]+?\]\])|(\[[^\]]+?\]\([^)]+\))/gi;

function appendInline(parent: HTMLElement, text: string) {
  appendInlineRaw(parent, text);
  // A table cell is rendered as a whole, so the inline-dataview pass has to run
  // on the result — that is where this vault keeps its `= this.field` queries.
  renderInlineDataview(parent, currentNotePath || null);
}

function appendInlineRaw(parent: HTMLElement, text: string) {
...
/**
 * like the predecessor interactive table editor: cells are click-to-edit, hover shows
...
// inline (the predecessor renders embedded HTML, e.g. CKEditor/Trilium tables, the same).
...
// Tags / cssclasses can't contain spaces (invalid in the predecessor) — collapse runs of
...
/** Parse the YAML subset the predecessor uses for properties. */
...
// the predecessor's built-in List properties.
const CORE_LIST_PROPS = new Set(['tags', 'aliases', 'cssclasses']);

// Property type → small glyph for the leading icon (display only, like the predecessor).
...
// The predecessor type id ↔ our display type.
...
class FrontmatterWidget extends WidgetType {
  ...
  toDOM(view: EditorView) {
    ...
    // Change a property's type: hand it to the server to keep, then convert the
    ...
    const makeScalarField = (dt: string, value: string): HTMLElement => {
      ...
      linkifyEditable(span);
      return span;
    };

    // Add an item to a list property via a typed input + suggestion dropdown
    // (vault tags for the `tags` property; free text otherwise), like the predecessor.
    ...
    props.forEach((p, idx) => {
      ...
      // Right-click the key or icon → Property type / Copy / Remove (like the predecessor).
      ...
      if (renderAsList) {
        for (const val of p.values) {
          ...
          txt.addEventListener('blur', () => {
            ...
            // Keep the raw value in step with the text: the link rendering below
            // paints from it, and painting a stale value would undo the edit.
            txt.dataset.raw = nv;
    ...
    // Add a property with a name-suggester dropdown (existing vault keys), like the predecessor.
    ...
    // The air below the block lives on a wrapper, not as a margin on the box:
    // the editor measures a block widget by its box and never sees a margin, so
    // a margin here shifts every click in the note by its height. The box keeps
    // its own border, which must stay tight under the last property row.
    const wrap = document.createElement('div');
    wrap.className = 'cm-props-wrap';
...
// The predecessor line/span classes (§20): HyperMD-header-N lines, cm-header-N spans.
...
function buildDecorations(view: EditorView): DecorationSet {
  ...
  for (const { from, to } of view.visibleRanges) {
    tree.iterate({
      ...
      enter: (node) => {
        ...
        // Code block lines: HyperMD-codeblock(-begin/-end) like the predecessor.
        ...
        // guide on the leading unit, like the predecessor.
        if (name === 'CodeBlock') {
        ...
        if (HEADING_LEVEL[name]) {
          ...
          // The space around a heading is the heading's business, not the
          // author's: whether there is a blank line above or below it in the
          // source is a habit of writing, not something a reader should see as
          // a difference. Both are drawn as slivers — still lines, still
          // clickable — and the heading's own padding sets the distance. That
          // is also what keeps it from ever sitting closer to the text above
          // than to the text below, which would read as the end of that text
          // rather than the start of what follows.
          if (line.number > 1) {
            const before = doc.line(line.number - 1);
            if (before.length === 0) {
              all.push(Decoration.line({ class: 'cm-blank-at-header' }).range(before.from, before.from));
              // The heading then needs almost no padding of its own: the blank
              // line above it already carries the distance. Without the mark it
              // would add its full gap on top and the space would double.
              all.push(Decoration.line({ class: 'cm-header-blank-above' }).range(line.from, line.from));
            }
            // A heading right under another heading is not a new section but the
            // same one being named more closely — a title and its subtitle. They
            // belong together, so the full section gap between them would say
            // the opposite of what is true.
            let n = line.number - 1;
            while (n > 1 && doc.line(n).length === 0) n--;
            if (/^#{1,6}\s/.test(doc.line(n).text) && n !== line.number) {
              all.push(Decoration.line({ class: 'cm-header-after-header' }).range(line.from, line.from));
            }
          }
          if (line.number < doc.lines) {
            const after = doc.line(line.number + 1);
            if (after.length === 0) {
              all.push(Decoration.line({ class: 'cm-blank-at-header' }).range(after.from, after.from));
            }
            // The first line under a heading does not need its own opening gap:
            // the heading is already the separation. Without this a list item
            // ends up further from the heading than the heading is from the
            // section above it, which reads the wrong way round.
            let n = line.number + 1;
            while (n <= doc.lines && doc.line(n).length === 0) n++;
            if (n <= doc.lines) {
              all.push(Decoration.line({ class: 'cm-first-under-header' }).range(doc.line(n).from, doc.line(n).from));
            }
          }
  ...
  for (const { from, to } of view.visibleRanges) {
    ...
    // The list item a following indented line belongs to: its text column and
    // how far it is indented. Lets a wrapped-over continuation line (an event's
    // description under `- Beschreibung`) line up with the item's text.
    let lastList: { hangCss: string; indentChars: number; depth: number } | null = null;
    ...
    for (let ln = startLine; ln <= endLine; ln++) {
      ...
      if (QUOTE_LINE_RE.test(text)) {
        ...
        if (cm) {
          ...
          // `> >` renders one bar per level like the predecessor.
          all.push(
            Decoration.line({
      ...
      // One indent unit = one tab or four spaces (matches indentGuides below).
      const indentUnits = listIndentUnits;
      ...
      // The indentation + marker is wrapped in a zero-width box that hangs into
      // the line's padding, so the text after it — and every wrapped line —
      // starts at the same x. (A negative text-indent would be the classic
      // trick, but Chromium then lays inline elements like links out at the
      // un-indented position, which drew them on top of the text.)
      const prefixMark = (len: number) => {
        // inclusiveStart keeps a widget that sits exactly at the start inside the
        // mark. Without it the bullet of a NON-indented item ends up outside and
        // stays in flow, which pushed the first line right of the wrapped ones.
        if (len > 0)
          all.push(
            Decoration.mark({ class: 'cm-list-prefix', inclusiveStart: true, inclusiveEnd: false }).range(bodyFrom, bodyFrom + len),
          );
      };
      ...
      if (task) {
        all.push(
          Decoration.line({
            ...
            // --hang = width of everything before the text (indent units +
            // checkbox + the space after it). The CSS turns it into a hanging
            // indent so wrapped lines line up under the text, not under the box.
            attributes: {
              'data-task': task[3],
              // Die Tiefe steht an der Zeile, damit die Gestaltung tiefer
              // liegende Punkte enger setzen kann als die oberste Ebene.
              'data-list-depth': String(indentUnits(task[1])),
              style: `--hang: calc(${indentUnits(task[1])} * var(--indent-unit) + 1.25em + 6px)`,
            },
          }).range(line.from, line.from),
        );
        lastList = { hangCss: `calc(${indentUnits(task[1])} * var(--indent-unit) + 1.25em + 6px)`, indentChars: task[1].length, depth: indentUnits(task[1]) };
        indentGuides(task[1]);
        prefixMark(task[0].length);
        ...
        if (bullet) {
          all.push(
            Decoration.line({
              class: 'HyperMD-list-line',
              attributes: {
                'data-list-depth': String(indentUnits(bullet[1])),
                style: `--hang: calc(${indentUnits(bullet[1])} * var(--indent-unit) + 1.1em)`,
              },
            }).range(line.from, line.from),
          );
          lastList = { hangCss: `calc(${indentUnits(bullet[1])} * var(--indent-unit) + 1.1em)`, indentChars: bullet[1].length, depth: indentUnits(bullet[1]) };
          indentGuides(bullet[1]);
          prefixMark(bullet[0].length);
          // Replace the `-`/`*`/`+` marker with a bullet and collapse its trailing
          // whitespace to a single space, so `-   Item` reads `• Item` like the predecessor.
          ...
          if (ol) {
            all.push(
              Decoration.line({
                class: 'HyperMD-list-line',
                // The marker keeps its digits, so its width grows with them.
                attributes: { style: `--hang: calc(${indentUnits(ol[1])} * var(--indent-unit) + ${ol[2].length * 0.55 + 0.3}em)` },
              }).range(line.from, line.from),
            );
            lastList = { hangCss: `calc(${indentUnits(ol[1])} * var(--indent-unit) + ${ol[2].length * 0.55 + 0.3}em)`, indentChars: ol[1].length, depth: indentUnits(ol[1]) };
            indentGuides(ol[1]);
            prefixMark(ol[0].length);
            if (ol[3].length > 1) {
            ...
            // No marker: an indented line under a list item continues it, so it
            // starts at that item's text column instead of at the margin.
            const cont = body.match(/^(\s+)(?=\S)/);
            if (lastList && cont && cont[1].length > lastList.indentChars) {
              all.push(
                Decoration.line({
                  class: 'HyperMD-list-line cm-list-continuation',
                  attributes: {
                    // Eine Fortsetzungszeile gehoert zum Punkt darueber.
                    'data-list-depth': String(lastList.depth),
                    style: `--hang: ${lastList.hangCss}`,
                  },
                }).range(line.from, line.from),
              );
              prefixMark(cont[1].length);
            } else if (!cont) {
              lastList = null; // back at the margin: the list is over
            }
          }
        }
      }
      ...
      while ((m = wikiRe.exec(text))) {
        ...
        if (isEmbed && /\.(bmp|png|jpe?g|gif|svg|webp|avif)$/i.test(href)) {
          ...
          // `![[clip.mp4]]` / `![[song.mp3]]` → native HTML5 player like the predecessor.
        ...
        } else if (isEmbed && /\.excalidraw(\.md)?$/i.test(href.split('#')[0])) {
          // `![[drawing.excalidraw]]` is a picture: painted, not transcluded.
          ...
          // form is reading-view behaviour and goes into aria-label like the predecessor.
      ...
      // Markdown escapes `\.` `\*` … — conceal the backslash like the predecessor LP
...
function buildFrontmatter(state: EditorState): DecorationSet {
  ...
  const end = fm[0].length;
  const blockEnd = text[end - 1] === '\n' ? end - 1 : end;
  // Always show the interactive Properties editor (like the predecessor); editing happens
...
/* ---------------- inline title (note filename, like the predecessor) ---------------- */
...
function buildInlineTitle(state: EditorState): DecorationSet {
  ...
  // export repeats the title as a heading, and the predecessor would otherwise show it twice.
...
function buildTables(state: EditorState): DecorationSet {
  ...
  const ro = state.field(livePreviewReadonly, false) ?? false;
  // Like the predecessor, tables are ALWAYS shown as the interactive widget (never as raw
...
export const livePreviewTheme = EditorView.baseTheme({
  // Heading sizes/weights come from the stylesheet (.cm-s-notes .HyperMD-header-N,
  ...
  '.cm-inline-title': {
    ...
    // Padding, not margin: the editor measures a block widget by its box, and a
    // margin here is height it never accounts for — the whole note's click-to-
    // caret mapping then sits a line off. Same rule as for .cm-line.
    margin: '0',
    padding: '0 0 0.5em',
  },
  ...
  // Links: accent-coloured and underlined like the predecessor. `inline-block` gives the
  // browser a soft-wrap opportunity after the widget, so text glued to `]]` in the
  // source (e.g. `]]and`) can still wrap onto the next line as it does in the predecessor.
  ...
  '.cm-props-wrap': { padding: '4px 0 18px' },
  '.cm-properties': { margin: '0' },
  ...
  // 24px padding. Callout colours live in notes.css (--callout-* slots).
  '.cm-line.cm-blockquote': {
    borderLeft: '2px solid var(--interactive-accent)',
    paddingLeft: '24px',
    color: 'var(--text-normal)',
  },
  // Markdown tables — mirror the predecessor's table CSS variables
  // (its documented CSS variables): 1px border,
  ...
  // Interactive table editor (like the predecessor): click-to-edit cells + hover controls.
