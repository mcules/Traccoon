/**
 * JSON Canvas — the open file format the predecessor's canvas uses (jsoncanvas.org).
 * A `.canvas` file is `{ "nodes": [...], "edges": [...] }`. We read/write it
 * verbatim so canvases stay round-trip compatible with the predecessor.
 */

export type CanvasSide = 'top' | 'right' | 'bottom' | 'left';
export type CanvasEnd = 'none' | 'arrow';
/** color = preset "1".."6" (the predecessor's palette) or a "#RRGGBB" hex string. */
...
export interface TextNode extends NodeBase {
  type: 'text';
  text: string;
  /** Our own extension (not in the JSON Canvas spec): horizontal text
   *  alignment for the card. Round-trips through our serializer; the real
   *  The predecessor ignores or strips it. Absent = left. */
...
/** Serialize to the pretty-printed JSON the predecessor writes (tab indent). */
export function serializeCanvas(data: CanvasData): string {
  return JSON.stringify({ nodes: data.nodes, edges: data.edges }, null, '\t');
}

/** 16-hex-char id, same shape the predecessor generates. `seed` keeps it deterministic. */
...
/** the predecessor's 6 preset accent colors (preset index "1".."6"). */
 ...
 * along each side's normal (the predecessor's smooth elbow look). Returns the SVG `d`
...
export function edgePath(
  ...
  // the curve — matches the predecessor. Sample the cubic and walk to half its length.
...
// ---- Alignment snapping (ported from the predecessor's canvas: getSnapping/O3/P3) ----
// While dragging nodes, the predecessor snaps the moving rects' edges & centers to the
...
/** A node's snap points: its 4 corners + center (the predecessor's P3). */
 ...
 * `dist` is the snap threshold in canvas units (the predecessor: ceil(15 / scale)).
