...
interface GNode {
  ...
  tags: string[];
  deg: number;
  fade?: number; // the predecessor's fadeAlpha: dims to 0.2 when another node is hovered
  x?: number;
  y?: number;
  vx?: number;
  vy?: number;
...
// --- force mapping: EXACT port of the predecessor's graph physics ------------------
// Reverse-engineered from the installed app (the packaged application → sim.js worker +
// app.js panel). The predecessor runs stock d3-force with: forceX/forceY gravity,
...
// Center slider → strength via the predecessor's MJ(e, .01) easing: 0.52 → 0.1.
...
// the predecessor's node sizing: getSize() = nodeSizeMult × clamp(3·√(deg+1), 8, 30).
const nodeRadius = (n: GNode, s: GraphSettings) =>
  s.nodeSize * Math.max(8, Math.min(3 * Math.sqrt(n.deg + 1), 30));
// the predecessor's renderer works in DEVICE pixels: scale 1 = 1 world unit per device
...
// the predecessor's zoom clamp (device scale) and node dim level for hover fade.
const SCALE_MIN = 1 / 128;
const SCALE_MAX = 8;
const FADE_DIM = 0.2;

/**
 * Graph view — WebGL-rendered via PixiJS (like the predecessor's PixiJS graph), with the
 ...
 * panel mirrors the predecessor: Tags / Attachments / Existing-only / Orphans, color
...
export default function GraphView() {
  ...
  // the predecessor's smooth zoom: wheel sets targetScale (device px), the render loop
  ...
  const [panelOpen, setPanelOpen] = useState(false); // the default of the predecessor: collapsed, cog only
  ...
  const getCols = (): ColorSet => {
    ...
    return {
      ...
      tag: 0x3aa757, // like the predecessor green for tag nodes
  ...
  const colorOf = (n: GNode, cols: ColorSet): number => {
    ...
    return cols.text; // The predecessor draws plain notes gray, not accent-colored
  };
  ...
  // One step of the predecessor's updateZoom(): lerp the scale 15% toward targetScale
  ...
  // centering the node, with the same 15%/frame lerp as the predecessor's updateZoom.
  ...
  // the predecessor's hover fade: nodes not linked to the highlighted node ease toward
  ...
  const doRender = () => {
    ...
    // sprites follow the predecessor's √zoom law — both need a refresh on layout
  ...
  const drawEdges = () => {
    ...
    const h = hover.current;
    const g = p.edges;
    // The predecessor draws edges at lineSizeMult / scale in world space — i.e. a
    ...
    ag.clear();
    if (s.arrows) {
      // The predecessor: arrow sprite scale = 2√(lineSizeMult)/scale (world units) and
  ...
  const ensureLabel = (i: number): Text => {
    ...
    let t = p.labels[i];
    if (!t) {
      // the predecessor's node label style: fontSize 14 (+ getSize()/4 applied via the
  ...
  const updateLabels = () => {
    ...
    const dpr = dprNow();
    // the predecessor's global text fade (device scale!): textAlpha =
    ...
    for (let ci = 0; ci < MAX; ci++) {
      ...
      // The predecessor: labels live in world space with scale = nodeScale and font
  ...
  // the predecessor's initial viewport: scale 1 in DEVICE pixels (no zoom-to-fit), the
  ...
  const resetCamera = () => {
    ...
    zoomTarget.current = 1; // device scale, like the predecessor's scale = targetScale = 1
  ...
  useEffect(() => {
    ...
    try {
      ...
      // The predecessor spawns every node at the origin and lets the sim "big-bang"
      ...
      // the predecessor's exact simulation (sim.js): d3-force with these params.
      simRef.current?.stop();
  ...
  useEffect(() => {
    ...
    sim.alpha(0.3).restart(); // The predecessor posts alpha .3 on force changes
    // eslint-disable-next-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    ...
    if (!canvas) return;
    // the predecessor's onWheel: targetScale ×= 1.5^(−ΔY/120) clamped to [1/128, 8]
...
// ----------------------------------------------------------------------------
// Filter panel (parity with the predecessor)
// ----------------------------------------------------------------------------
