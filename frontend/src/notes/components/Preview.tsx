...
import { renderDataviewBlocks } from '../lib/dataview';
...
export default function Preview({ source }: { source?: string }) {
  ...
  useEffect(() => {
    ...
    renderMarkdown(content, {
      ...
      resolveEmbed: (target) =>
        memo('embed-preview', target, async () => {
          try {
            const { path } = await api.resolve(target);
            if (!path) return null;
            const r = await api.read(path);
            const raw = typeof r === 'string' ? r : r.content;
            return { path, content: extractEmbedSection(raw, target) };
          } catch {
            return null;
          }
        }),
  ...
  // Inline title (note filename), like the predecessor — skipped when the note already
