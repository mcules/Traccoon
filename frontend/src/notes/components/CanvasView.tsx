...
// the predecessor's object-snap distance is 15px at 100%; it grows as you zoom out.
const SNAP_BASE = 15;
...
export default function CanvasView() {
  ...
  // Alignment guide lines shown while dragging nodes (the predecessor's canvas-snaps).
  ...
  // Right-click a node → like the predecessor context menu with many actions.
  ...
  // Set arrow direction on the selected edges (the predecessor's dropdown):
  ...
  // The predecessor editor hotkeys, on the plain textarea (parity with the main editor's
  // notesKeymap): ⌘B bold · ⌘I italic · ⌘K add link · ⌘L task · ⌘/ comment.
  ...
  // The predecessor editor context-menu structure (mined from the packaged application i18n).
  ...
  const onViewportPointerDown = (e: React.PointerEvent) => {
    // Middle- or right-drag pans from anywhere (parity with the predecessor), even over a node.
  ...
  // Grab an existing edge's endpoint and drag it out (reconnect / extend), like the predecessor.
  ...
  const onViewportPointerMove = (e: React.PointerEvent) => {
    ...
    if (dr.mode === 'move') {
      let dx = c.x - dr.cx;
      let dy = c.y - dr.cy;
      // Shift while moving locks to the dominant axis (parity with the predecessor).
  ...
  const onWheel = (e: React.WheelEvent) => {
    // Plain wheel zooms toward the cursor, like the predecessor's canvas.
    const r = vpRef.current!.getBoundingClientRect();
    ...
    >
      <div className="canvas-world" style={worldStyle}>
        ...
        {data.nodes.filter((n) => n.type !== 'group').map((n) => {
          ...
          // While dragging a connection, reveal anchors on EVERY node (like the predecessor's
        ...
        {/* Alignment guides (the predecessor's canvas-snaps) — drawn while dragging nodes */}
        {snap && (snap.x || snap.y) && (
      ...
      {/* Selection menu — parity with the predecessor's canvas (Remove / Set color /
      ...
      {/* Drop-an-edge-on-empty menu (Add card / Add note from vault), like the predecessor */}
      {edgeDrop && (
        <div
      ...
      {/* Text-card formatting menu (right-click inside an editing card) — parity with the predecessor */}
      {textMenu && (
...
/** Hierarchical text-card formatting menu (parity with the predecessor). Buttons preventDefault
